...
Stripe.setPublishableKey("pk_test_6chGmDIc3ftpiOH7C5bQl6pW");
...
var stripeResponseHandler = function stripeResponseHandler(status, response) {
  ...
  if (response.error) {
    ...
    // $form.append($('<input type="hidden" name="cart" />').val(cart.getCart()));
    var jsonString = JSON.stringify(cart.getCart());
    $form.append($("<input type=\"hidden\" name=\"cart\" />").val(jsonString));
